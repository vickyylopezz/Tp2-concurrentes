...
use std::{net::{UdpSocket, SocketAddr, IpAddr, Ipv4Addr}, thread, sync::Arc};
use tp2::{
...
fn get_coffee_machines(socket: Arc<UdpSocket>, addr: SocketAddr) -> Vec<Addr<CoffeeMachine>> {
    ...
    for i in 0..COFFEE_MACHINES {
        ...
        coffee_makers.push(CoffeeMachine { id: i, server_addr: addr, socket: socket.clone() }.start());
    }
...
fn handle_client(socket: UdpSocket, num_oders: u32) {
    ...
    loop {
        ...
        let (size, client_addr) = socket.recv_from(&mut buf).expect("Error when receiving data");
...
fn main() -> Result<(), Error> {
    ...
    let socket = Arc::new(UdpSocket::bind(socket_addr).expect("Error when binding server socket"));
